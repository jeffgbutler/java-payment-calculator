...
The application uses Spring Boot's autoconfiguration for Redis, which is based on properties.
...
The application expects to be bound to a Redis service instance named "java-calculator-redis" on Cloud Foundry. You can
...
translate Redis credentials into the proper environment variables for Spring Boot autoconfiguration. Magic!
...
2. Push image to Docker Hub (login to Docker Hub first with `docker login`):

   ```shell
   docker push jeffgbutler/payment-calculator
   ```

3. Create a network in Docker so the payment calculator can find Redis by DNS name:
...
5. Start the Payment Calculator in Docker:
...
After testing, clean up Docker by following these steps:

1. Run `docker ps` to get the container IDs for Redis and the payment calculator
2. Run `docker stop <image_id>` for each image ID
3. Run `docker system prune` to remove the stopped images, and the network (you can also use `docker system prune -a` to
...
After testing, clean up your cluster with the following commands:
